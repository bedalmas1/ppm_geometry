...
A second, independently-discovered constraint forced a further cut: this project's execution environment terminates long-running background training processes at approximately the 60-minute mark. A first attempt at a 40-epoch budget was killed mid-run at the ~60-minute mark while still actively training and improving its checkpoint (confirmed via the checkpoint file's timestamp, 8 minutes before the kill) — not a crash of its own. **`epochs` was cut to 15** (~34 minutes at the measured rate, comfortably under that limit) — the same cap this project used for A6 LUPIN's own comparably-expensive full-suffix model, keeping the two directly comparable. `patience=30` (from the paper) is kept as configured but never triggers within a 15-epoch budget, so training always runs the full 15 epochs — the same outcome as A6.

### Results

Trained on Helpdesk, same split as A1–A6 (confirmed identical `dataset_split_hash`). Training was smooth and monotonic — validation loss dropped every single epoch (1.4022 → 0.7113 across all 15 epochs, no early stop triggered, matching A6's own "ran the full capped budget" outcome). **Overall val 0.9200 / test 0.8034** mean normalized DL-similarity (hard-argmax, beam-size-1 greedy generation, matching every other full-suffix model's evaluation protocol in this roster).

**Unlike A4/A5/A6, a genuine published-number comparison exists here**: MLMME's own SDM 2021 paper *does* evaluate Helpdesk (its Table 2: average SDL **0.8411**, beam size 1, its own 7:1:2 chronological split and preprocessing — not this project's own stricter, independently-derived split). This project's test-set result (0.8034) lands reasonably close to that published number, though somewhat below it — plausible, non-exclusive explanations: (1) this project's activity-only scope removes the duration-time input/output entirely, while the published number's model retained it (a joint multi-task objective can sometimes regularize the shared LSTM representations, so removing it is not guaranteed to only help); (2) this project's 15-epoch budget is a **3% sliver** of the paper's own 500-epoch budget — by a wide margin the smallest fraction of its originally-reported training budget of any full-suffix model in this roster (compare: A4 early-stopped at epoch 43/200 ≈ 21.5%, A5 at epoch 25/500 ≈ 5%, A6 ran 15/50 = 30%) — so some remaining gap to the published number is very plausibly attributable to genuine undertraining, not a pipeline or reproduction defect; (3) this project's own stricter chronological split (vs. the paper's own, differently-derived 7:1:2 split) is a separate, independently-motivated source of difference, discussed next.

### The val/test gap, now confirmed a SEVENTH time — the largest gap yet among full-suffix models, but not necessarily meaningful

A1 (86.9%→72.9%), A2 (81.8%→63.5%), A3 (86.4%→72.1%), A4 (0.924→0.816), A5 (0.917→0.816), A6 (0.926→0.829), now **A7 (0.9200→0.8034)** — seven models, seven architectures, two objectives, two metric families, one dataset/split, one still-consistent qualitative pattern (validation always exceeds test). A7's gap (0.1166) is the **largest** of the four full-suffix DL-similarity models measured so far (A4: 0.108, A5: 0.101, A6: 0.097, A7: 0.117) — this **breaks** the narrowing trend that had appeared across A4→A5→A6, rather than continuing or extending it. Given A7 also received, by a wide margin, the smallest fraction of its own paper's recommended training budget (15/500 = 3%, versus A6's 30%, A4's ~21.5%, A5's ~5%) among this batch, the most likely explanation is undertraining relative to the other models rather than a genuine architecture-driven generalization difference — this should **not** be read as evidence against A6's "pretrained-LLM encoders generalize more robustly" hypothesis, since A7's shorter relative training budget confounds any such comparison. Worth revisiting only if a longer MLMME run (outside this project's current background-process time constraint) becomes practical later.
...
- `results/helpdesk/mlmme/{checkpoint.pt, val_metrics_by_prefix_length.csv, test_metrics_by_prefix_length.csv, manifest.json}` (gitignored)
