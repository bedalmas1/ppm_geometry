...
B1 controlled Transformer (next-event) - see
paper/phase4_representation_extraction.md for the full schema/design.

Usage:
    uv run --extra torch python experiments/extract_controlled_transformer_next.py [dataset]

Does not retrain: reloads from this model's own
results/<dataset>/controlled_transformer_next/manifest.json, following the
same "reload from manifest, no retrain" pattern already established by
...
(src/representations/cache.py).

z_t extraction point: model.encode_zt(...) - the last non-padded position's
hidden state, gathered from ControlledTransformerEncoder's per-position
output (src/models/controlled_transformer/model.py).
"""
...
MODEL_NAME = "controlled_transformer_next"
...
def main() -> None:
    ...
    tokens, pad_mask, lengths, _labels = encode(test_prefixes, vocab, max_case_length)
    ...
    with torch.no_grad():
        for i in range(0, len(test_prefixes), batch_size):
            ...
            pad = pad_mask[i : i + batch_size].to(DEVICE)
            length = lengths[i : i + batch_size].to(DEVICE)
            z_t = model.encode_zt(x, pad, length)
