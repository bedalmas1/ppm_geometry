#!/usr/bin/env python
"""Train & evaluate A5 CRTP-LSTM (full-suffix, activity-only, direct
non-autoregressive prediction) on one dataset.

Usage:
    uv run --extra torch python experiments/train_crtp_lstm.py configs/experiments/crtp_lstm_helpdesk.yaml

Unlike A4 SuTraN, no autoregressive generation is needed here (see
model.py's docstring) - a single forward pass yields the full suffix
prediction at both train and eval time, using the SAME masked
cross-entropy loss/evaluation code path (just no teacher-forced decoder
input to construct). Vocabulary/target-encoding reused directly from A4's
adapter (models.sutran.adapter) - same task, same shared suffix-prefix
definition (data.prefixes.make_suffix_prefixes).
"""
...
from models.crtp_lstm.adapter import (  # noqa: E402
    build_reverse_class_dict,
    build_vocab,
    encode_prefixes_left_padded,
    encode_training_suffixes,
    get_window_size,
)
from models.crtp_lstm.model import CRTPLSTMActivityOnly  # noqa: E402
...
def masked_val_loss(model, x, y, loss_fn, batch_size=256) -> float:
    model.eval()
    total, n = 0.0, 0
    with torch.no_grad():
...
def evaluate_direct(model, prefix_df, vocab, x, eos_class, batch_size=256) -> tuple[pd.DataFrame, float]:
    """Direct (single forward pass) suffix prediction + normalized DL
    similarity per instance, grouped by prefix length k."""
    model.eval()
    ...
    with torch.no_grad():
        for i in range(0, len(prefix_df), batch_size):
            xb = x[i : i + batch_size].to(DEVICE)
            logits = model(xb)
            pred_classes = logits.argmax(dim=-1).cpu().numpy()  # (batch, window_size)
...
def main() -> None:
    ...
    train_x = encode_prefixes_left_padded(train_prefixes, vocab, window_size)
    _, train_y = encode_training_suffixes(train_prefixes, vocab, window_size)
    val_x = encode_prefixes_left_padded(val_prefixes, vocab, window_size)
    _, val_y = encode_training_suffixes(val_prefixes, vocab, window_size)
    test_x = encode_prefixes_left_padded(test_prefixes, vocab, window_size)

    model = CRTPLSTMActivityOnly(
        vocab_size=vocab.vocab_size,
        num_classes=vocab.num_classes,
        d_model=model_config["d_model"],
        dropout=model_config["dropout"],
        num_shared_lstm_layers=model_config["num_shared_lstm_layers"],
        num_dedicated_lstm_layers=model_config["num_dedicated_lstm_layers"],
    ).to(DEVICE)
    ...
    train_loader = DataLoader(TensorDataset(train_x, train_y), batch_size=model_config["batch_size"], shuffle=True)

    result_dir = REPO_ROOT / "results" / dataset_name / "crtp_lstm"
    ...
    for epoch in (range(start_epoch, total_epochs) if not stopped_early else []):
        ...
        val_loss = masked_val_loss(model, val_x, val_y, loss_fn)
    ...
    val_per_k, overall_val_dl_similarity = evaluate_direct(model, val_prefixes, vocab, val_x, vocab.eos_class)
    test_per_k, overall_test_dl_similarity = evaluate_direct(model, test_prefixes, vocab, test_x, vocab.eos_class)
