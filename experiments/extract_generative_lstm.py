...
A2 GenerativeLSTM - see paper/phase4_representation_extraction.md for the
full schema/design.

Usage:
    uv run --extra tf python experiments/extract_generative_lstm.py [dataset]

Does not retrain: reloads from this model's own
results/<dataset>/generative_lstm/manifest.json (no-retrain pattern from
...
z_t extraction point: the "prefix_representation" final LSTM layer
(src/models/generative_lstm/model.py), same not-exposed-by-default situation
as A1 - built as a second functional-model output here. Unlike A1, the final
Dense head already uses activation="softmax", so model.predict's second
output is already a probability distribution (no extra softmax needed).
"""
...
def main() -> None:
    ...
    z, probs = extractor.predict(token_x, verbose=0)
