...
A5 CRTP-LSTM - see paper/phase4_representation_extraction.md for the full
schema/design.

Usage:
    uv run --extra torch python experiments/extract_crtp_lstm.py [dataset]

Does not retrain: reloads from this model's own
results/<dataset>/crtp_lstm/manifest.json (no-retrain pattern from
...
z_t extraction point: model.encode(prefix_tokens_left_padded)[:, -1, :] -
because the input is left-padded, position window_size-1 always corresponds
to the last real prefix event, regardless of actual prefix length
(src/models/crtp_lstm/model.py). Unlike A4 SuTraN, no autoregressive
decoding is needed: one forward pass (model(x)) already returns logits for
every suffix position at once, so "prediction probabilities"/entropy are
just position 0 of that same forward pass (the first future event) -
predicted_suffix uses every position, truncated at the first predicted EOS,
same convention as train_crtp_lstm.py's own evaluate_direct.
"""
...
from models.crtp_lstm.adapter import encode_prefixes_left_padded  # noqa: E402
...
def main() -> None:
    ...
    prefix_tokens = encode_prefixes_left_padded(test_prefixes, vocab, window_size)
